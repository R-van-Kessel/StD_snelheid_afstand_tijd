...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        // Kies wat we zoeken: 1=snelheid, 2=afstand, 3=tijd
        const questionType = floor(random(1, 4));
        
        let distance, time, speed;
        let distanceUnit, timeUnit, speedUnit;
        let text, answer;
        
        // Kies random snelheid eenheid
        speedUnit = random() > 0.5 ? 'm/s' : 'km/h';
        
        if (questionType === 1) {
            // ZOEK SNELHEID
            if (speedUnit === 'm/s') {
                // Genereer afstand en tijd
                const useMeters = random() > 0.5;
                
                if (useMeters) {
                    distanceUnit = 'm';
                    distance = floor(random(100, 5000));
                    
                    const useSeconds = random() > 0.5;
                    if (useSeconds) {
                        timeUnit = 'sec';
                        time = floor(random(10, 300));
                        speed = distance / time;
                    } else {
                        timeUnit = 'min';
                        time = floor(random(1, 20));
                        speed = distance / (time * 60);
                    }
                } else {
                    distanceUnit = 'km';
                    distance = (floor(random(1, 50)) * 10) / 10;
                    
                    const useMinutes = random() > 0.5;
                    if (useMinutes) {
                        timeUnit = 'min';
                        time = floor(random(5, 60));
                        speed = (distance * 1000) / (time * 60);
                    } else {
                        timeUnit = 'uur';
                        time = (floor(random(1, 10)) * 10) / 10;
                ...
                if (useKm) {
                    distanceUnit = 'km';
                    distance = floor(random(5, 200));
                    
                    const useHours = random() > 0.5;
                    if (useHours) {
                        timeUnit = 'uur';
                        time = (floor(random(1, 20)) * 10) / 10;
                        speed = distance / time;
                    } else {
                        timeUnit = 'min';
                        time = floor(random(10, 120));
                        speed = distance / (time / 60);
                    }
                } else {
                    distanceUnit = 'm';
                    distance = floor(random(500, 10000));
            ...
            text = `afstand: ${distance.toString().replace('.', ',')} ${distanceUnit}\ntijd: ${time.toString().replace('.', ',')} ${timeUnit}\nsnelheid\nin ${speedUnit} = ?`;
            answer = speed.toString().replace('.', ',');
            
        } else if (questionType === 2) {
            // ZOEK AFSTAND
            // Genereer snelheid en tijd, bereken afstand
            
            if (speedUnit === 'm/s') {
                speed = floor(random(5, 50)); // 5-50 m/s
                
                // Kies afstand-eenheid voor antwoord
                distanceUnit = random() > 0.5 ? 'm' : 'km';
                ...
                if (useSeconds) {
                    ...
                    let distanceInMeters = speed * time;
                    ...
                    timeUnit = 'min';
                    time = floor(random(1, 20));
                    let distanceInMeters = speed * time * 60;
                ...
                // km/h
                speed = floor(random(20, 150)); // 20-150 km/h
                
                distanceUnit = random() > 0.5 ? 'km' : 'm';
                
                const useHours = random() > 0.5;
                if (useHours) {
                    timeUnit = 'uur';
                    time = (floor(random(1, 10)) * 10) / 10;
                    let distanceInKm = speed * time;
                    ...
                    timeUnit = 'min';
                    time = floor(random(5, 60));
                    let distanceInKm = speed * (time / 60);
            ...
            text = `snelheid: ${speed} ${speedUnit}\ntijd: ${time.toString().replace('.', ',')} ${timeUnit}\nafstand in ${distanceUnit} = ?`;
            answer = distance.toString().replace('.', ',');
            
        } else {
            // ZOEK TIJD
            // Genereer snelheid en afstand, bereken tijd
            
            if (speedUnit === 'm/s') {
                speed = floor(random(5, 50));
                
                distanceUnit = random() > 0.5 ? 'm' : 'km';
                
                // Kies tijd-eenheid voor antwoord
                timeUnit = random() > 0.5 ? 'sec' : 'min';
                ...
                if (distanceUnit === 'm') {
                    ...
                    if (timeUnit === 'sec') {
                        ...
                        time = Math.round((timeInSeconds / 60) * 100) / 100;
                    }
                } else {
                    distance = (floor(random(1, 20)) * 10) / 10;
                    let timeInSeconds = (distance * 1000) / speed;
                ...
                // km/h
                speed = floor(random(20, 150));
                
                distanceUnit = random() > 0.5 ? 'km' : 'm';
                timeUnit = random() > 0.5 ? 'uur' : 'min';
                ...
                if (distanceUnit === 'km') {
                    ...
                    if (timeUnit === 'uur') {
                        time = Math.round(timeInHours * 100) / 100;
                    } else {
                        time = Math.round(timeInHours * 60);
                    }
                } else {
                    distance = floor(random(500, 10000));
                    let timeInHours = (distance / 1000) / speed;
            ...
            text = `snelheid: ${speed} ${speedUnit}\nafstand: ${distance.toString().replace('.', ',')} ${distanceUnit}\ntijd in ${timeUnit} = ?`;
            answer = time.toString().replace('.', ',');
        }
